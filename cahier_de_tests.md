...
Ce document consigne les résultats d'exécution de la suite de tests Playwright décrite dans [plan_de_tests.md](plan_de_tests.md).
Source des résultats : rapport HTML Playwright (`playwright-report/index.html`), exécution du 27/08/2026 à 01:49:16 (durée totale : 2.0 min, projet `chromium`).
...
- [Détail des échecs (corrélés aux bugs connus du plan de test)](#détail-des-échecs-corrélés-aux-bugs-connus-du-plan-de-test)
  ...
  - [Transverse (1 échec)](#transverse-1-échec)
...
|:-----:|:------------:|:-----------:|:-----------------:|:-------:|
|  244  | 199 (81,6 %) | 45 (18,4 %) |         0         |    0    |

**À retenir : la quasi-totalité des 45 échecs ne sont pas des régressions de la suite de tests, mais la matérialisation attendue des bugs déjà recensés dans le plan de test** pour les comptes `problem_user`, `error_user`, `performance_glitch_user`, `visual_user` et `locked_out_user`. Le taux de réussite de 81,6 % reflète donc surtout le volume de scénarios dédiés à documenter ces comportements défaillants du site, plus qu'un défaut de fiabilité des tests eux-mêmes.
...
| [transverse.spec.ts](tests/transverse.spec.ts)                   |   36    |   35    |   1    |      97,2 %      |
| **Total**                                                        | **244** | **199** | **45** |    **81,6 %**    |

## Détail des échecs (corrélés aux bugs connus du plan de test)

### Login (1 échec)

- `locked_out_user` : le cookie `session-username` est tout de même créé malgré le blocage du compte. Il s'agit d'un BUG connu, documenté et annoté dans le test (`login.spec.ts:24`).
...
Le bug "ajout d'un ID inconnu modifie le panier" est **transverse à tous les comptes** (déjà noté au plan de test, section *ItemDetail > Accès à un ID inconnu*).
...
| Item dupliqué en LocalStorage affiché en 2 cartes distinctes plutôt qu'une carte avec quantité 2 | (tous comptes, cas générique) |     1     |

### Basket (2 échecs)

- `standard_user` : le bouton "Checkout" reste actif avec un panier vide et redirige quand même, il s'agit d'un BUG identifié pendant l'élaboration du plan de test.
- `standard_user` : le badge du panier reste visible/affiché malgré un ID inconnu sans item correspondant, il s'agit d'un BUG identifié pendant l'élaboration du plan de test.

### Transverse (1 échec)

- `visual_user` : le bouton "Checkout" de l'écran Basket n'est pas correctement positionné (classe `btn_visual_failure`). Il s'agit d'un BUG identifié pendant l'élaboration du plan de test.

### CheckoutComplete (0 échec)

Tous les scénarios passent (9/9) : confirmation de commande, génération du PDF et navigation post-achat sont stables sur les comptes couverts (`standard`, `performance_glitch`, `visual`).

## Conclusion

Sur les 244 tests exécutés, les 45 échecs recensés correspondent à des comportements défectueux du site déjà anticipés et documentés dans le [plan de test](plan_de_tests.md) pour les comptes de démonstration bugués (`problem_user`, `error_user`, `performance_glitch_user`, `visual_user`, `locked_out_user`).
